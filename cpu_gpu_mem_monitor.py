...
class CpuGpuMemoryInfo(object):
    ...
    def monitor(self, args):
        ...
        while True:
            ...
            if (self.sample_nums % 5 == 0 and self.sample_nums != 0):
                ...
                # self.logger.info("save figure success, sample count = {}, now time = {}".
    ...
    def draw_gpu_image(self, data_path, fitting):
        ...
        self.logger.info("finished draw gpu image")
        # 设置gpu坐标轴范围
    ...
    def draw_mem_image(self, data_path, fitting):
        ...
        self.logger.info("min memory = %d" % min(y_list))
        self.logger.info("max memory = %d" % max(y_list))
        self.logger.info("avg memory = %d" % np.mean(y_list))
        ...
        # 如果是手动画图, 则memory的范围从config.json中读取
        if self.mode == "manual":
            self.total_mem_size = self.config_dict["memory"]["max_mem_size"]
        self.mem_subplot.set_ylim(0, self.total_mem_size)
        # 设置图例位置,loc可以为[upper, lower, left, right, center]
        self.mem_subplot.legend(loc='right', shadow=True)
        self.logger.info("finished draw memory image")
    ...
    def run(self, args):
        ...
        try:
            ...
            plt.savefig(os.path.join(self.save_dir,
                                     "monitor-result-fitting.png"), bbox_inches='tight')
            self.logger.info("save to %s" % self.save_dir)
            self.logger.info("total run time: {} s".format(
